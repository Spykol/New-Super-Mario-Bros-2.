		...
		class Asteroid
		{
			...
			*/
			constructor()
			{
				...
				this.explosionSound = createAudio("game-assets/powerdown.wav");
				this.detonateSound = createAudio("game-assets/stomp.wav");
			...
			load()
			{
				...
				*/
				this.y = 100 + Math.floor(Math.random() * 400); // random X position
			...
			display()
			{
				image(this.image, this.x-= this.asteroidSpeed, this.y); // each time an asteroid is displyed its position is updated (asteroids fall [y is increased])
				if (this.x < 1) // asteroid is dissappeared and load() is called to load again the asteroid
				{
					this.load();
					return true;
				}
			...
			detonate()
			{
				if (!this.exploded) // each asteroid can explode/detonate only once
				{
					...
					this.detonateSound.play(); // explosion sound
		...
		class MissilePack
		{
			...
			constructor()
			{
				...
				this.loadSound = createAudio("game-assets/powerup.wav");
			}
			...
			newMissilePack(score)
			{
				...
				{
					let xR = Math.floor(Math.random() * 700);
					if ( xR == 1 )
					{
						console.log('Create a missile pack');
						this.x = 60; // random X position
						this.y = 0;
			...
			checkForCollection(spaceship) // if a missilepack collides with the spaceship is collected
			{
				...
				if (Math.abs(this.y - spaceship.y) < 50)
				{
		...
		class Missile
		{
			...
			display()
			{
				...
				image(this.image, this.x+=15, spaceship.y); // missiles move forward (y is decreased)
				if (this.x > 1200) // missile is dissappeared
				{
					return false;
				}
				return true;
		...
		class AsteroidSwarm
		{
			...
			*/
			...
			handleDifficulty()
			{
				...
				if (this.asteroids.length < (this.asteroidsPassed/15))
			...
			checkForCollision(spaceship)
			{
				...
				for (let i = 0; i < this.asteroids.length; i++)
				{
					if (Math.abs(this.asteroids[i].x - spaceship.x) < 30 && (this.asteroids[i].y - spaceship.y) < 100 && (this.asteroids[i].y - spaceship.y) > -10 )
					{
			...
			checkForDetonation(missiles)
			{
				...
				for (let i = 0; i < this.asteroids.length; i++)
				{
					...
					{
						if (Math.abs(this.asteroids[i].x - missiles[z].x) < 10  && Math.abs(this.asteroids[i].y - missiles[z].y) < 150 && Math.abs(this.asteroids[i].y - missiles[z].y) > -50 )
						{
							missiles[z].explode();
		...
		class SpaceShip
		{
			...
			constructor()
			{
				...
				this.engineSound = createAudio("game-assets/maintheme.wav");
			}
			...
			move(move)
			{
				if (this.y > 115 && move < 0) // chech that will not get out of the left barrier
				{
					this.y += (move*10);
				}

				if (this.y < 423 && move > 0) // chech that will not get out of the right barrier
				{
					this.y += (move*10);
				}
			}
		...
		*/
		function preload()
		{
			...
			asteroidImage = loadImage("game-assets/bill.png"); // load once and the pass to Asteroid so that will not load each time an Asteroid is created
			...
			gameSounds
			$.get('game-assets/mariosounds.pd', function(patchStr) {
		...
		function draw()
		{
			...
			*/
			...
			if (!gameOver && startGame && !paused) // while the game is played
			{
				...
				if (keyIsDown(38)) // left arrow is pressed
				{
					spaceship.move(-1);
				}

				if (keyIsDown(40)) // right arrow is pressed
				{
					spaceship.move(1);
				}
		...
		{
			textSize(30);
			text("Score: " + asteroidSwarm.asteroidsPassed, 980, 40); // Score is shown

			text("FireBalls: " + spaceship.missiles, 1120, 40); // Score is shown
			...
			if (!startGame || gameOver) // provide instructions
			{
				...
				textSize(25);
				text('Use the up and down arrows to avoid the bullet bills.', 340, 360, 800, 200);
				text('Get the fire flower and use it by pressing the Space Bar!', 340, 390, 800, 200);
			}
